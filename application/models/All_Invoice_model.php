...
class All_Invoice_model extends CI_Model{
	...
	function _get_applicant_query($isActive="", $PeriodMonth="", $PeriodYear="", $InvType="", $InvStsID=""){
        ...
        $__column_search    = array('InvID', 'InvType', 'PeriodYear', 'PeriodMonth', 'InvNo', 'PONo', 'PO_Type', 'AgencyName', 'AdvertiserName', 'ProductName', 'AE_Name', 'Gross', 'AgencyDisc', 'Nett', 'InvStsName', 'EntryBy_date');
        $__column_order     = array('InvID', 'InvType', 'PeriodYear', 'InvNo', 'AgencyName', 'AE_Name', 'Gross', 'AgencyDisc', 'Nett', 'InvStsName', 'EntryBy_date');
     ...
     if (empty($InvType) == FALSE) {
        $InvType = ($InvType == 'M' ? 0 : 1);
         $this->db->where('InvType', $InvType);
     }
